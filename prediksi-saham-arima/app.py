    ...
    if data_df is None or data_df.empty:
        ...
        with st.expander("Lihat Analisis Diagnostik Awal"):
            ...
            if adf_result[1] > 0.05:
                st.warning('⚠️ **Data tidak stasioner.** Sebaiknya gunakan `d > 0`.')
            else:
                st.success('✅ **Data stasioner.** Anda mungkin bisa menggunakan `d = 0`.')
        ...
        if run_button:
            with st.spinner('Melatih model...'):
                try:
                    ...
                    # =======================================================================
                    # --- SEMUA PERINGATAN PENTING DIKEMBALIKAN DI SINI ---
                    with st.expander("Lihat Detail Validasi Model"):
                        st.text(str(results.summary()))
                        
                        # Cek signifikansi parameter
                        param_p_values = results.pvalues[1:] # Abaikan konstanta
                        if (param_p_values > 0.05).any():
                            st.error('❌ **Parameter Tidak Signifikan:** Satu atau lebih parameter AR/MA memiliki p-value > 0.05. Model ini mungkin terlalu kompleks atau tidak tepat.')
                        else:
                            st.success('✅ **Signifikansi Parameter OK:** Semua parameter model signifikan secara statistik.')

                        # Cek residual white noise (Uji Ljung-Box)
                        lb_test = acorr_ljungbox(results.resid, lags=[10], return_df=True)
                        lb_p_value = lb_test['lb_pvalue'].iloc[0]
                        
                        st.write(f'**Hasil Uji Ljung-Box pada Residual:** P-value = `{lb_p_value:.4f}`')
                        if lb_p_value < 0.05:
                            st.error('❌ **Residual Tidak White Noise:** Masih ada pola autokorelasi pada residual. Model belum menangkap semua informasi.')
                        else:
                            st.success('✅ **Residual White Noise:** Tidak ada autokorelasi signifikan yang terdeteksi pada residual.')
                    # =======================================================================
